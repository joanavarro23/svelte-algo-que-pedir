...
describe('+page.ts load - detalle pedido', () => {
  ...
  describe('Manejo de errores', () => {
    it('lanza error 404 cuando el pedido no existe', async () => {
      vi.mocked(detalleService.obtenerDetallePedido).mockRejectedValue({
        response: { status: 404 }
      })
      ...
      try {
        ...
        expect((error as { body: { message: string } }).body.message).toContain('no encontrado')
      }
    })

    it('lanza error 500 cuando falla el servidor', async () => {
      vi.mocked(detalleService.obtenerDetallePedido).mockRejectedValue(
